...
const Education: React.FC = () => {
  ...
  const EducationCard = ({ item, isPreview = false }: { item: EducationItem; isPreview?: boolean }) => (
    <div className={`education-card bg-background/80 backdrop-blur-sm border border-border rounded-xl p-4 sm:p-8 lg:p-10 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 ${isPreview ? 'border-accent/20' : ''}`}>
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-3 sm:mb-5">
        <div className="flex-1">
          <h4 className={`${isPreview ? 'text-lg sm:text-2xl' : 'text-xl sm:text-3xl'} font-bold text-foreground mb-2 sm:mb-4`}>{item.title}</h4>
          <p className="text-accent font-semibold mb-2 sm:mb-4 text-base sm:text-xl">{item.institution}</p>
          <div className="flex items-center text-muted-foreground mb-2 sm:mb-4">
            <Calendar className="w-4 sm:w-6 h-4 sm:h-6 mr-2 sm:mr-4" />
            <span className="text-sm sm:text-lg">{item.date}</span>
          </div>
        </div>
        {item.grade && (
          <div className="flex items-center bg-accent/10 text-accent px-3 sm:px-5 py-1.5 sm:py-2.5 rounded-full text-sm sm:text-lg font-medium mt-2 sm:mt-0 w-fit">
            <Award className="w-4 sm:w-6 h-4 sm:h-6 mr-1.5 sm:mr-2" />
            {item.grade}
          </div>
        )}
      </div>
      ...
      {item.type === 'certification' && (
        <div className="mt-4 sm:mt-6">
          <button
            ...
            className="inline-flex items-center px-4 sm:px-5 py-2 sm:py-3 bg-accent hover:bg-accent/90 text-white rounded-lg text-sm sm:text-lg font-medium transition-all duration-300 hover:scale-105 shadow-sm"
          >
            <FileText className="w-4 sm:w-6 h-4 sm:h-6 mr-2 sm:mr-3" />
            Ver Certificado
          </button>
        </div>
      )}
  ...
  return (
    <section id="estudios" className="education-section py-12 sm:py-16 lg:py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-6 sm:mb-12 lg:mb-16">
          ...
          <p className="text-base sm:text-xl lg:text-2xl text-muted-foreground px-4 sm:px-0">Mi trayectoria educativa y crecimiento profesional</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 sm:gap-8 lg:gap-10">
          ...
          <div className="lg:col-span-2">
            <div className="education-grid space-y-5 sm:space-y-8 lg:space-y-10">
              {/* Education Section */}
              <div>
                <div className="flex items-center justify-between mb-4 sm:mb-8">
                  <div className="flex items-center">
                    <GraduationCap className="w-5 sm:w-7 lg:w-8 h-5 sm:h-7 lg:h-8 text-accent mr-2 sm:mr-3" />
                    <h3 className="text-lg sm:text-2xl lg:text-3xl font-bold text-foreground">Estudios</h3>
                  </div>
                </div>
                ...
                {/* Show All Education */}
                {showAllEducation && (
                  <div className="space-y-3 sm:space-y-5 mb-3 sm:mb-5">
                ...
                <button
                  ...
                  className="flex items-center text-accent hover:text-accent/80 font-medium transition-colors duration-300 text-xs sm:text-base lg:text-lg"
                >
                  {showAllEducation ? 'Ver menos' : `Ver todos los estudios (${educationItems.length})`}
                  <ChevronRight className={`w-3 sm:w-5 h-3 sm:h-5 ml-1 sm:ml-2 transition-transform duration-300 ${showAllEducation ? 'rotate-90' : ''}`} />
                </button>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-4 sm:mb-8">
                  <div className="flex items-center">
                    <FileText className="w-5 sm:w-7 lg:w-8 h-5 sm:h-7 lg:h-8 text-accent mr-2 sm:mr-3" />
                    <h3 className="text-lg sm:text-2xl lg:text-3xl font-bold text-foreground">Certificaciones</h3>
                  </div>
                </div>
                ...
                >
                  ...
                  <ChevronRight className={`w-3 sm:w-5 h-3 sm:h-5 ml-1 sm:ml-2 transition-transform duration-300 ${showAllCertifications ? 'rotate-90' : ''}`} />
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-1">
            <div className="sidebar-content lg:sticky lg:top-24 space-y-4 sm:space-y-8">
              {/* Key Competencies */}
              <div className="bg-background/80 backdrop-blur-sm border border-border rounded-xl p-4 sm:p-6 lg:p-8 shadow-lg">
                <div className="flex items-center mb-3 sm:mb-5">
                  <Target className="w-5 sm:w-7 lg:w-8 h-5 sm:h-7 lg:h-8 text-accent mr-2 sm:mr-3" />
                  <h3 className="text-sm sm:text-lg lg:text-xl font-bold text-foreground">Competencias Clave</h3>
                </div>
                ...
                <div className="space-y-1.5 sm:space-y-3">
                  {skills.map((skill, index) => (
                    <div
                      key={index}
                      className="flex items-center p-1.5 sm:p-3 rounded-lg hover:bg-accent/10 transition-colors duration-300"
                    >
                      <div className="w-1.5 sm:w-2.5 h-1.5 sm:h-2.5 bg-accent rounded-full mr-2 sm:mr-4 flex-shrink-0"></div>
                      <span className="text-muted-foreground text-xs sm:text-base">{skill}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Continuous Learning */}
              <div className="bg-gradient-to-br from-accent/10 to-secondary/10 border border-accent/20 rounded-xl p-4 sm:p-6 lg:p-8">
                <div className="flex items-center mb-3 sm:mb-5">
                  <BookOpen className="w-5 sm:w-7 lg:w-8 h-5 sm:h-7 lg:h-8 text-accent mr-2 sm:mr-3" />
                  <h3 className="text-sm sm:text-lg lg:text-xl font-bold text-foreground">Aprendizaje Continuo</h3>
                </div>
                ...
                <div className="mt-3 sm:mt-5 p-2.5 sm:p-4 bg-accent/20 rounded-lg">
                  <p className="text-accent text-xs sm:text-base font-medium">
