...
const Education: React.FC = () => {
  ...
  const EducationCard = ({ item, isPreview = false }: { item: EducationItem; isPreview?: boolean }) => (
    <div className={`education-card bg-background/80 backdrop-blur-sm border border-border rounded-xl p-4 sm:p-5 lg:p-6 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 ${isPreview ? 'border-accent/20' : ''}`}>
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-3 sm:mb-4">
        <div className="flex-1">
          <h4 className={`${isPreview ? 'text-lg sm:text-xl' : 'text-xl sm:text-2xl'} font-bold text-foreground mb-2`}>{item.title}</h4>
          <p className="text-accent font-semibold mb-2 text-base sm:text-lg">{item.institution}</p>
        ...
        {item.grade && (
          <div className="flex items-center bg-accent/10 text-accent px-3 py-1.5 rounded-full text-sm font-medium mt-2 sm:mt-0 w-fit">
    ...
    <section id="estudios" className="education-section py-8 sm:py-12 lg:py-20">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="text-center mb-8 sm:mb-12 lg:mb-16">
          ...
          <p className="text-base sm:text-lg lg:text-xl text-muted-foreground px-4 sm:px-0">Mi trayectoria educativa y crecimiento profesional</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8 max-w-6xl mx-auto">
          ...
          <div className="lg:col-span-1">
            <div className="sidebar-content lg:sticky lg:top-24 space-y-6 sm:space-y-8">
              {/* Key Competencies */}
              <div className="bg-background/80 backdrop-blur-sm border border-border rounded-xl p-4 sm:p-5 lg:p-6 shadow-lg mx-2 sm:mx-0">
              ...
              {/* Continuous Learning */}
              <div className="bg-gradient-to-br from-accent/10 to-secondary/10 border border-accent/20 rounded-xl p-4 sm:p-5 lg:p-6 mx-2 sm:mx-0">
                <div className="flex items-center mb-4 sm:mb-5">
