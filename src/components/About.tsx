...
const About: React.FC = () => {
  ...
  return (
    <section id="sobre-mi" className="about-section min-h-screen flex items-center py-12 sm:py-16 lg:py-20">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        <div className="text-center mb-6 sm:mb-12 lg:mb-16">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-foreground mb-4 sm:mb-6">Sobre Mí</h2>
          <p className="text-base sm:text-xl lg:text-2xl text-muted-foreground px-4 sm:px-0">Conoce más sobre mi experiencia y pasión por el desarrollo</p>
        </div>
        ...
        <div className="chat-container bg-background/80 backdrop-blur-md rounded-2xl border border-border shadow-2xl overflow-hidden" 
             style={{ height: 'min(calc(100vh - 6rem), 650px)' }}>
          {/* Chat Header */}
          <div className="bg-accent/10 px-4 sm:px-8 lg:px-10 py-4 sm:py-6 border-b border-border">
              ...
              <div className="flex items-center space-x-3 sm:space-x-5">
                <div className="relative">
                  <img
                    src="./marcos.png"
                    alt="Marcos"
                    className="w-10 sm:w-14 lg:w-16 h-10 sm:h-14 lg:h-16 rounded-full border-2 border-accent/20"
                  />
                </div>
                <div>
                  <h3 className="font-semibold text-foreground text-base sm:text-xl lg:text-2xl">Marcos</h3>
                  <p className="text-sm sm:text-lg text-green-500 flex items-center font-medium">
                    <span className="w-2.5 sm:w-3.5 h-2.5 sm:h-3.5 bg-green-500 rounded-full mr-2 sm:mr-4 animate-pulse"></span>
          ...
          {/* Chat Messages */}
          <div id="chat-messages" className="flex-1 overflow-y-auto p-4 sm:p-8 lg:p-10 space-y-4 sm:space-y-6" 
            ...
            {messages.map((message) => (
              ...
              >
                <div
                  className={`max-w-[85%] sm:max-w-[80%] lg:max-w-md xl:max-w-lg px-3 sm:px-4 lg:px-5 py-3 sm:py-4 rounded-2xl shadow-sm ${
                    message.isBot
                      ? 'bg-card text-foreground border border-border'
                      : 'bg-accent text-white'
                  }`}
                >
                  <p className="text-sm sm:text-lg leading-relaxed">{message.text}</p>
                  <p className={`text-xs sm:text-base mt-2 sm:mt-4 ${message.isBot ? 'text-muted-foreground' : 'text-white/70'}`}>
                    {message.timestamp}
                  </p>
                </div>
              </div>
          ...
          {currentOptions.length > 0 ? (
            <div className="p-3 sm:p-6 lg:p-8 border-t border-border bg-muted/20">
              <p className="text-xs sm:text-base text-muted-foreground mb-2 sm:mb-4">Pregúntame sobre:</p>
              <div className="flex flex-wrap gap-2 sm:gap-3">
                {currentOptions.slice(0, 3).map((option) => (
                  <button
                    key={option.id}
                    onClick={() => handleOptionClick(option)}
                    className="chat-option inline-flex items-center px-3 sm:px-5 lg:px-6 py-2 sm:py-3 bg-accent/10 hover:bg-accent hover:text-white text-accent rounded-full text-xs sm:text-base font-medium transition-all duration-300 hover:scale-105 border border-accent/20"
                  >
                    {option.text}
                  </button>
                ))}
            ...
            <div className="p-3 sm:p-6 lg:p-8 border-t border-border bg-gradient-to-r from-accent/10 to-green-500/10">
              <div className="flex flex-wrap gap-3 justify-center">
                <button
                  onClick={scrollToContact}
                  className="inline-flex items-center px-4 sm:px-6 py-2.5 sm:py-3 bg-accent hover:bg-accent/90 text-white rounded-lg text-xs sm:text-base font-medium transition-all duration-300 hover:scale-105 shadow-sm"
                >
                  Ir a Contacto
                </button>
              </div>
